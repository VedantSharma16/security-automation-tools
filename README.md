...
| Project | Description |
|---|---|
| [`secret-scanner/`](secret-scanner/) | Git-aware scanner for hardcoded secrets and credential leaks: 19 regex rules for known secret formats (AWS, GitHub, Slack, Stripe, private keys, JWTs...), a Shannon-entropy heuristic for everything else, and a git-history mode that catches secrets committed and later "removed." Redacts every value it reports, supports a `.secretsallowlist` for false positives, and exits non-zero on findings for CI gating. 100 pytest tests. |
