...
| Project | Description |
|---|---|
| [`soc-agent/`](soc-agent/) | Agentic orchestrator that routes freeform incident evidence to the tools below, runs them as subprocesses, and merges the results into one consolidated triage report — routing and the analyst narrative are heuristic/template by default, or Claude-backed (tool-calling) with `--llm`. Ties the rest of the repo together into a single pipeline. Fully tested with pytest; works offline by default. |
