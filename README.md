...
| Project | Description |
|---|---|
| [`attack-surface-mapper/`](attack-surface-mapper/) | Parses `nmap` XML scans, correlates open services/versions against a local CVE and insecure-protocol rule database, scores and ranks exposure per host, and produces a prioritized pentest-style report with an optional LLM-generated narrative grounded strictly in the structured findings. Fully tested with pytest; works offline by default. |
