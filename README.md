...
| Project | Description |
|---|---|
| [`attack-surface-mapper/`](attack-surface-mapper/) | Authorized external recon: passively enumerates subdomains via certificate-transparency logs, then (only once you confirm `--i-am-authorized`) actively audits each resolved host for open sensitive ports, missing HTTP security headers, and weak/expiring TLS certificates, rolling everything up into a per-host risk score. The offensive/recon counterpart to the blue-team tools below. Fully tested offline against mocked network/socket/TLS layers. |
