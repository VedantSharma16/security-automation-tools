...
| Project | Description |
|---|---|
| [`recon-agent/`](recon-agent/) | Agentic, read-only attack-surface recon: a bounded think-act-observe loop probes HTTPS/HTTP/TLS on an authorized host, analyzes responses for security-header and transport-security misconfigurations, and produces a prioritized report. Deterministic offline policy by default, optional LLM-driven (Anthropic tool use) policy. Fully tested with pytest; no external network access required to run the suite. |
