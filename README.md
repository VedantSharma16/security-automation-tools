...
| [`recon-agent/`](recon-agent/) | Agentic, passive attack-surface recon assistant: an LLM tool-use loop that adaptively calls DNS/subdomain/HTTP-header/TLS/WHOIS probes, backed by a deterministic offline pipeline and a rules-based findings/severity engine when no API key is set. |
