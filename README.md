...
| Project | Description |
|---|---|
| [`web-recon-auditor/`](web-recon-auditor/) | Passive HTTP attack-surface recon for authorized pentest targets: security-header analysis, sensitive-path exposure probing with soft-404 baselining, and TLS posture (weak protocol / cert expiry), with a grounded LLM executive summary and a deterministic offline fallback. Requires an explicit `--authorized` flag to run. Fully tested with pytest against a local HTTP server fixture. |
