...
| [`secret-sentinel/`](secret-sentinel/) | Secrets/credential-leak scanner: regex signatures for known vendor key formats (AWS, GitHub, Slack, Stripe, ...), Shannon-entropy analysis for everything else, git-history scanning (catches secrets removed from the working tree but still in old commits), an allowlist baseline, and optional LLM-assisted triage of ambiguous findings — grounded only in redacted metadata, never the raw secret. |
