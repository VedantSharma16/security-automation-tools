...
| Project | Description |
|---|---|
| [`incident-response-agent/`](incident-response-agent/) | A ReAct-style, tool-calling agent that investigates a security incident by deciding for itself which local tools to call — CVE, IP, and domain reputation lookups, ATT&CK keyword matching — one step at a time, then synthesizes a severity verdict with a full reasoning trace. Ships both a deterministic offline planner and a live Anthropic tool-use planner (with automatic fallback). Fully tested with pytest, including a fake-client harness for the multi-turn LLM loop. |
