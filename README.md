...
| Project | Description |
|---|---|
| [`recon-agent/`](recon-agent/) | Agentic passive-recon assistant: a genuine LLM tool-calling loop (not a fixed pipeline) drives from-scratch DNS/HTTP/TLS recon primitives against an authorized target, scores the findings, and produces a report. Deterministic offline fallback when no API key is set; fully tested offline with mocked network points. |
