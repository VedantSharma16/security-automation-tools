...
| Project | Description |
|---|---|
| [`soc-alert-agent/`](soc-alert-agent/) | Autonomous, tool-calling SOC alert triage agent: an actual ReAct-style agentic loop (via the Anthropic Messages API's native tool use) decides which investigative tool to call next — IOC reputation, asset criticality, alert correlation, ATT&CK lookup — until it reaches an escalate/monitor/close verdict, with a full step-by-step audit trail. A deterministic offline planner drives the identical loop when no LLM is available. Fully tested with pytest (LLM loop tested via a mocked `anthropic` module); works offline by default. |
