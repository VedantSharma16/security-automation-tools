...
| Project | Description |
|---|---|
| [`soc-agent/`](soc-agent/) | Agentic SOC alert-triage playbook runner. A Claude tool-use loop autonomously decides which internal systems to query (asset inventory, threat intel, identity risk, EDR process baseline) based on which alert fields are present, then submits a scored verdict with an auditable investigation trace. A deterministic offline planner runs the same tool functions when no API key is set, so the pipeline is fully testable without one. |
