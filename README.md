...
| [`recon-assistant/`](recon-assistant/) | Offensive-security counterpart to the triage tools above: an async TCP port scanner with banner grabbing, passive service fingerprinting (cleartext protocols, outdated banners, exposed databases/admin surfaces), an HTTP security-header audit, and a risk-scored report with an optional LLM narrative. Refuses to scan non-private targets by default unless authorization is explicitly confirmed. A professional rewrite of the earlier `port_scanner.py` experiment below. |
| [`port_scanner.py`](port_scanner.py) | Basic TCP connect-scan of common ports using the `socket` library. Superseded by `recon-assistant/`, kept here for history. |
