...
| Project | Description |
|---|---|
| [`llm-redteam-scanner/`](llm-redteam-scanner/) | Automated adversarial testing framework for LLM-backed apps: runs a 12-attack prompt-injection/jailbreak battery (direct override, roleplay, obfuscation, indirect RAG-context injection, secret extraction, excessive agency) mapped to the OWASP Top 10 for LLM Applications, judges results via canary-leak detection + compliance heuristics with optional LLM-judge refinement, and reports severity. Ships with a naive vs. hardened demo target pair to show the contrast. Fully tested with pytest; works offline by default. |
