...
| Project | Description |
|---|---|
| [`soc-agent/`](soc-agent/) | An agentic incident-investigation assistant: a from-scratch plan/act/observe tool-calling loop that adaptively decides which checks to run (threat intel, process reputation, asset criticality, user-baseline deviation, ATT&CK mapping) based on what earlier steps found, then scores severity and produces a narrative. Ships a deterministic offline planner plus an optional Claude tool-use planner. Fully tested with pytest; works offline by default. |
