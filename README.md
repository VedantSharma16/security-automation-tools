...
| Project | Description |
|---|---|
| [`soc-agent/`](soc-agent/) | Agentic SOC alert-triage assistant: a genuine ReAct-style tool-calling loop where Claude autonomously chooses which investigation tools to call (threat-intel lookup, DNS/registration, MITRE ATT&CK, log search, asset criticality) and when to stop, with a deterministic offline planner fallback using the same tools. Contrasts with the fixed pipeline below. Fully tested with pytest; works offline by default. |
