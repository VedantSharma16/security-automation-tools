...
| [`httpsec-auditor/`](httpsec-auditor/) | Passive, zero-dependency web app security auditor: checks security headers, cookie flags, CORS misconfiguration (reflected-origin/wildcard-with-credentials probing), and TLS cert/protocol/cipher strength against a target, then produces a scored (A–F), remediation-oriented report. Every analyzer is a pure function tested offline against synthetic data; 59 unit tests. |
