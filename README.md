...
| [`secret-scanner/`](secret-scanner/) | Offline AppSec/DevSecOps tool: scans a working tree *and* full git history (via diff parsing) for leaked credentials, using vendor-format regex rules plus a Shannon-entropy generic fallback. Supports a fingerprint-based baseline/allowlist workflow for CI gating, with redacted output and JSON/table reporting. Zero runtime dependencies; 60-test pytest suite. |
