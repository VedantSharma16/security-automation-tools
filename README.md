...
| Project | Description |
|---|---|
| [`attack-surface-mapper/`](attack-surface-mapper/) | Passive external recon tool: DNS/email-security posture (SPF/DMARC), certificate-transparency subdomain discovery, HTTP security-header auditing, and TLS certificate/protocol inspection, rolled into a 0-100 risk-scored report with an optional LLM narrative. The offensive-recon counterpart to the blue-team tools below. Fully tested with pytest via injectable network clients; no live network access needed to run the suite. |
