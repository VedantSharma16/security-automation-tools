...
| [`pentest-report-agent/`](pentest-report-agent/) | Parses Nmap XML / JSON scan findings, applies CVSS-lite risk scoring and OWASP Top 10 mapping from a local knowledge base, and generates an executive summary via an **agentic LLM tool-calling loop** (with a deterministic offline fallback) that looks up remediation guidance and per-host business criticality on its own. Markdown/JSON output, CI-friendly exit codes, fully tested with pytest. |
