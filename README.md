...
| [`attack-surface-mapper/`](attack-surface-mapper/) | Passive web recon/attack-surface triage for authorized assessments: security-header audit, robots.txt/sitemap recon, tech fingerprinting, and an opt-in curated exposure-path probe, scored into a 0-100 risk rating. An agentic Claude tool-use loop decides which checks to run (deterministic offline fallback included) — a different LLM pattern than the RAG retrieval used in `ioc-triage-assistant`. |
