...
| Project | Description |
|---|---|
| [`recon-agent/`](recon-agent/) | Agentic recon tool: a planner (Claude native tool-use, or a deterministic offline fallback) decides step-by-step which read-only recon tool to call next against an authorized target — port scan, banner grab, HTTP header fetch, TLS cert inspection — fingerprints services, and flags known CVEs from a local knowledge base. Authorization-gated (private/loopback only unless explicitly confirmed), fully tested with local TCP/TLS test servers and a mocked LLM client. |
