...
| Project | Description |
|---|---|
| [`web-security-auditor/`](web-security-auditor/) | Passively audits HTTP security headers, cookie hardening, and TLS certificate posture for one or more URLs, scores them A–F against OWASP Secure Headers / Mozilla Observatory rules, and can gate a CI pipeline on the result. Zero runtime dependencies (stdlib only); 42 tests run against a local test server and mocked sockets, no external network calls. |
