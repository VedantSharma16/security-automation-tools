...
| Project | Description |
|---|---|
| [`recon-agent/`](recon-agent/) | Passive attack-surface recon (DNS hygiene, HTTP security headers, TLS certificate health, robots.txt disclosures) for authorized targets, scored into an A-F posture grade. Runs a fixed deterministic tool order by default, or an agentic mode where Claude drives a native tool-use loop to choose which checks to run and writes the analyst summary itself. Every tool's network call is dependency-injected, so the 53-test suite runs fully offline. |
