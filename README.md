...
| Project | Description |
|---|---|
| [`codesec-scanner/`](codesec-scanner/) | Static application security scanner (SAST) for Python codebases: AST-based detection of insecure patterns (SQL/command injection, unsafe deserialization, weak crypto, disabled TLS verification, debug mode) plus regex/entropy-based hardcoded-secret detection, a `--fail-on` severity gate for CI, and an optional LLM remediation narrative grounded strictly in the structured findings. Fully tested with pytest; works offline by default. |
