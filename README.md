...
| [`phishing-triage-agent/`](phishing-triage-agent/) | Agentic tool-calling pipeline that investigates a raw `.eml` message: the LLM adaptively decides which investigation tools to run (headers, SPF/DKIM/DMARC, URL/typosquat reputation, attachments, urgency language) and in what order via native Anthropic tool-use, then submits a verdict with cited evidence. Falls back to a deterministic full-sweep scorer offline; fully tested without any API key. |
