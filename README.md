...
| Project | Description |
|---|---|
| [`soc-agent/`](soc-agent/) | Agentic SOC investigator: runs a Claude tool-calling (ReAct-style) loop over a raw alert, letting the model decide which of six tools to call (IP/domain reputation, WHOIS age, GeoIP, process baseline, ATT&CK search) before reaching a verdict — with a full auditable thought/action/observation trace and a deterministic offline fallback that exercises the same tools without an API key. |
