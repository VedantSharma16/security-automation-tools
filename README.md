...
| [`websec-scanner/`](websec-scanner/) | Authorized-use web application security scanner: security-header/cookie-flag analysis, sensitive-file exposure probes, passive tech fingerprinting, and opt-in active reflected-XSS/SQLi probes via a same-origin crawler. JSON/Markdown reporting, 38 pytest tests run against a real local vulnerable Flask target. |
