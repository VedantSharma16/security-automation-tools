...
| Project | Description |
|---|---|
| [`recon-agent/`](recon-agent/) | Agentic, read-only attack-surface reconnaissance for authorized assessments: a plan-act-observe loop (offline deterministic policy, or LLM-driven tool selection) runs DNS/subdomain enumeration, dangling-CNAME subdomain takeover detection, a concurrent port scan, and an HTTP security-header + TLS certificate audit, then scores findings and writes an executive summary. Stdlib-only core, 49 tests, no real network calls in the test suite. |
