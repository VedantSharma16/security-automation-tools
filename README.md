...
| Project | Description |
|---|---|
| [`pentest-report-builder/`](pentest-report-builder/) | Parses nmap `-sV` scan output, matches services against a curated local vulnerability knowledge base (real CVEs: vsftpd backdoor, Heartbleed, EternalBlue, BlueKeep, etc.), scores per-host and engagement-wide risk, and generates a client-ready pentest report with an optional LLM executive summary grounded strictly in the structured findings. The red-team/reporting counterpart to the blue-team tools below. Fully tested with pytest; works offline by default. |
