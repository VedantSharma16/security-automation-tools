...
| Project | Description |
|---|---|
| [`pentest-report-agent/`](pentest-report-agent/) | Merges Nmap scan output with manually-recorded findings, dedupes/scores/prioritizes them via an agentic tool-use pipeline (a deterministic offline agent, or a real Claude tool-use loop over the same tools), and renders a client-ready pentest report with an executive summary. Fully tested with pytest; works offline by default. |
