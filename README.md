...
| [`http-security-auditor/`](http-security-auditor/) | Passive AppSec/pentest-recon tool: audits a target URL's security headers, cookie flags, CORS config, and TLS health, rolls findings into a weighted 0-100 score and A-F grade, and generates an LLM remediation narrative (with an offline fallback). Stdlib-only core, fully offline-testable, fully tested with pytest. |
