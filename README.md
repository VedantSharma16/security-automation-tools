...
| Project | Description |
|---|---|
| [`secops-agent/`](secops-agent/) | Autonomous, tool-using SOC investigation agent: given a natural-language question, it decides for itself which of four sandboxed, read-only tools to call (log search, IOC lookup, MITRE ATT&CK lookup, asset-criticality lookup) and in what order, using each result to inform the next call, until it reaches a verdict. Real multi-turn Claude tool-use loop plus a fully offline deterministic planner; guardrails include a tool allowlist, a log-path sandbox, and an iteration budget. Fully tested with pytest, including a scripted fake Claude client. |
