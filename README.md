...
| Project | Description |
|---|---|
| [`phishing-email-analyzer/`](phishing-email-analyzer/) | Parses raw `.eml` files and runs rule-based phishing heuristics — SPF/DKIM/DMARC auth failures, Reply-To mismatches, brand impersonation and typosquat sender domains, link cloaking, raw-IP/shortened links, urgency language, and dangerous/masquerading attachments — into a 0-100 risk score and verdict, with an optional LLM-generated analyst narrative grounded strictly in the structured findings. Fully tested with pytest; works offline by default. |
