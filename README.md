...
| [`web-security-auditor/`](web-security-auditor/) | Passive HTTP security posture scanner (securityheaders.com-style) — grades a site's security headers, cookie flags, and TLS configuration (deprecated protocols, cert expiry) on an A+–F scale. Batch-scans a URL list, exits non-zero below a configurable grade for CI gating. 79 offline tests (network layer fully mocked). |
