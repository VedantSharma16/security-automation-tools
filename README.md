...
| Project | Description |
|---|---|
| [`agentic-soc-copilot/`](agentic-soc-copilot/) | An autonomous tool-calling SOC triage agent: rather than a fixed pipeline, the model (via Anthropic tool-use) decides which of four tools to call — threat-intel lookup, LOLBin/process reputation, MITRE ATT&CK retrieval, asset criticality — observes each result, and concludes with a schema-validated verdict. A deterministic offline planner mirrors the same loop for network-free use and testing, including a mocked-client test suite for the tool-calling logic itself. |
