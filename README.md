...
| Project | Description |
|---|---|
| [`secrets-scanner/`](secrets-scanner/) | Git-aware secret-detection tool: regex signatures + Shannon-entropy detection for hardcoded credentials, git-history scanning (catches secrets deleted from the working tree but still in history), baseline suppression for reviewed false positives, and optional LLM-assisted triage of ambiguous findings — with a fully offline fallback. Redacts every matched secret at the source; raw values are never stored, printed, or persisted. |
