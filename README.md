...
| [`web-recon-scanner/`](web-recon-scanner/) | Single-target web recon CLI: security-header analysis, TLS/certificate health checks, header/HTML technology fingerprinting, `robots.txt`/sitemap review, and optional concurrent DNS subdomain brute-forcing, rolled into a weighted risk score. Requires an explicit `--i-own-this-target` acknowledgment before it will run. Fully tested with pytest against mocked HTTP/DNS. |
