...
| [`recon-agent/`](recon-agent/) | Authorized-recon pipeline (DNS/subdomain enum, port scan, HTTP/TLS fingerprinting) orchestrated by a real Claude tool-use loop, with a deterministic offline fallback and rule-based risk findings (exposed services, admin panels, missing security headers, expiring certs). Refuses to scan non-private targets without an explicit authorization flag. Fully tested with pytest. |
