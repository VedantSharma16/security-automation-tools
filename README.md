...
| [`websec-auditor/`](websec-auditor/) | Passive web security posture auditor (pentest-recon-flavored): grades a target's HTTP security headers, cookie flags, and TLS/certificate config (Mozilla-Observatory-style A-F score), with lightweight tech fingerprinting, JSON/Markdown reports, and a `--fail-on` CI gate. Fully tested offline (59 tests) against fake HTTP/TLS responses. |
