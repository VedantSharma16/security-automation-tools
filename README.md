...
| [`websec-recon/`](websec-recon/) | Authorized external recon tool: subdomain enumeration, threaded TCP port scanning, HTTP security header analysis, and TLS certificate checks, merged into a single scored report (console/Markdown/JSON). Stdlib only; 47 tests, all offline via injected fakes for every network call. |
