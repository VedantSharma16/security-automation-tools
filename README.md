...
| [`web-security-auditor/`](web-security-auditor/) | Web app recon/assessment tool: checks security headers, cookie flags, CORS misconfiguration, TLS config, and sensitive-path exposure against a target URL, scores the result, and maps every finding to an OWASP Top 10 category. Zero required deps (stdlib `urllib`/`ssl` only), fully offline-testable, optional LLM-generated executive narrative. |
