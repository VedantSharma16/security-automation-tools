...
| Project | Description |
|---|---|
| [`soc-agent-orchestrator/`](soc-agent-orchestrator/) | Agentic investigation pipeline that sits above the three tools below: given a case directory of evidence, a real Claude tool-use loop (with a deterministic offline fallback) decides which specialist tools apply, runs them, and merges their output into one severity/risk/MITRE/recommended-actions verdict. Each specialist tool is called the way an analyst would, via its own CLI, keeping all four projects fully decoupled. 48 tests, works fully offline by default. |
