...
| Project | Description |
|---|---|
| [`web-recon-agent/`](web-recon-agent/) | A ReAct-style agentic pipeline for authorized web-app recon: an agent loop repeatedly picks the next lightweight tool (DNS, headers, security-header grading, robots.txt, TLS, a short port scan) based on the transcript so far, then produces a risk-scored report. Deterministic offline planner by default; an optional Claude tool-use planner lets the model choose the recon path dynamically. Fully tested with pytest against a local loopback HTTP server; no network access required. |
