...
| Project | Description |
|---|---|
| [`phish-forensics/`](phish-forensics/) | Offline-first phishing/BEC email triage: parses raw `.eml` files, evaluates SPF/DKIM/DMARC, extracts and classifies IOCs (defanged), detects brand-impersonation via typosquat/homoglyph matching against a local brand list, flags disguised links and dangerous/double-extension attachments, and produces a scored report with an optional LLM analyst narrative. Stdlib-only core, 49 tests, no network calls or attachment execution in the pipeline. |
