...
| Project | Description |
|---|---|
| [`web-security-auditor/`](web-security-auditor/) | Passive external web app security scanner: security headers, cookie flags, TLS certificate health, and common exposed-path misconfigurations (`.git/HEAD`, `.env`, backup files), aggregated into a risk-scored report. GET-only, no exploitation, with a CLI authorization gate. Fully tested offline via mocked HTTP/TLS. |
